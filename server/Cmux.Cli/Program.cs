...
/// <summary>
/// cmux CLI — talks to the running cmux3 web host over HTTP/REST.
/// The base URL defaults to http://localhost:5201 and can be overridden
/// with the CMUX_URL environment variable.
///
/// Usage:
...
public static class Program
{
    private static readonly string BaseUrl =
        (Environment.GetEnvironmentVariable("CMUX_URL") ?? "http://localhost:5201").TrimEnd('/');
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        {
            Console.Error.WriteLine($"Error: Could not connect to cmux at {BaseUrl}. Is it running?");
            return 1;
        }
        catch (TaskCanceledException)
        {
            Console.Error.WriteLine($"Error: Request to cmux at {BaseUrl} timed out.");
            return 1;
        }
        catch (Exception ex)
        {
    ...
    {
        if (args.Length == 0) { Console.Error.WriteLine("Usage: cmux workspace <list|create|select|next|previous>"); return 1; }
    ...
    {
        if (args.Length == 0) { Console.Error.WriteLine("Usage: cmux surface <create|next|previous>"); return 1; }
        var sub = args[0].ToLowerInvariant();
        var state = await GetState();
        var ws = FindSelectedWorkspace(state);
    ...
    {
        if (args.Length == 0) { Console.Error.WriteLine("Usage: cmux split <right|down>"); return 1; }
    ...
    {
        Console.WriteLine("""
            cmux - Terminal workspace CLI (cmux3 web host)

            Usage:
              cmux <command> [options]

            Environment:
              CMUX_URL              Base URL of the host (default http://localhost:5201)
    ...
    {
        Console.WriteLine("cmux3 1.0.0 (web)");
        return 0;
    }
