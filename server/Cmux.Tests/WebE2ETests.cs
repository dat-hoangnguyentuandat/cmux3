...
/// <summary>
/// End-to-end tests for the cmux3 web backend: REST API for workspaces,
/// surfaces and split panes, plus the live terminal WebSocket. These verify
/// the web app reproduces the cmux2 workspace/surface/split/terminal behavior.
/// </summary>
public class WebE2ETests : IClassFixture<WebApplicationFactory<Program>>
{
    private readonly WebApplicationFactory<Program> _factory;
    private static readonly JsonSerializerOptions J = new(JsonSerializerDefaults.Web);

    public WebE2ETests(WebApplicationFactory<Program> factory)
    {
        // Use a clean, isolated state file per test process.
        var dir = Path.Combine(Path.GetTempPath(), "cmux3-e2e-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(dir);
        Environment.SetEnvironmentVariable("LOCALAPPDATA", dir);
        var statePath = Path.Combine(dir, "cmux3", "state.json");
