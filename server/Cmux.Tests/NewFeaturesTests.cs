...
public class WorkspaceTemplateServiceTests : IDisposable
{
    ...
    public WorkspaceTemplateServiceTests()
    {
        ...
        _tempDir = Path.Combine(Path.GetTempPath(), $"cmux-test-templates-{Guid.NewGuid():N}");
    ...
    {
        // Use the real service — it writes to %LOCALAPPDATA%\cmux2\templates
...
public class ExternalAgentServiceTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"cmux-test-agent-{Guid.NewGuid():N}");
        Directory.CreateDirectory(_tempDir);
    }
...
public class WorkspaceTemplateFileTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"cmux-tmpl-{Guid.NewGuid():N}");
        Directory.CreateDirectory(_tempDir);
    }
