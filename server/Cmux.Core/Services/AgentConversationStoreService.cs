...
public sealed class AgentConversationStoreService
{
    private static readonly string RootDir = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "cmux2", "agent");
    ...
    public IReadOnlyList<AgentConversationThread> GetAllThreads()
    {
        lock (_lock)
        {
            return _threads.Values
    ...
    public bool DeleteMessage(string threadId, string messageId)
    {
        lock (_lock)
        {
            ...
            int idx = messages.FindIndex(m => string.Equals(m.Id, messageId, StringComparison.Ordinal));
            if (idx < 0)
                return false;
            ...
            {
                sb.Append(JsonSerializer.Serialize(m, JsonLineOptions));
                sb.Append(Environment.NewLine);
            }
            File.WriteAllText(path, sb.ToString(), Encoding.UTF8);

            thread.MessageCount = Math.Max(0, thread.MessageCount - 1);
            thread.TotalInputTokens = Math.Max(0, thread.TotalInputTokens - Math.Max(0, removed.InputTokens));
            thread.TotalOutputTokens = Math.Max(0, thread.TotalOutputTokens - Math.Max(0, removed.OutputTokens));
            thread.TotalTokens = Math.Max(0, thread.TotalTokens - Math.Max(0, removed.TotalTokens));
            if (removed.IsCompactionSummary)
                thread.CompactionCount = Math.Max(0, thread.CompactionCount - 1);

            var last = messages.LastOrDefault();
            if (last != null)
            {
                thread.UpdatedAtUtc = last.CreatedAtUtc;
                var preview = (last.Content ?? "").Replace("\r", "", StringComparison.Ordinal)
                    .Replace("\n", " ", StringComparison.Ordinal)
                    .Trim();
                if (preview.Length > 160)
                    preview = preview[..160] + "...";
    ...
    public bool DeleteThread(string threadId)
    {
        lock (_lock)
        {
            ...
            {
                // Ignore filesystem errors; index is already updated.
            }
