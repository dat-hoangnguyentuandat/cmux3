...
/// <summary>
/// Manages command snippets with JSON persistence to <c>%LOCALAPPDATA%/cmux2/snippets.json</c>.
/// Seeds useful defaults on first use.
/// </summary>
public class SnippetService
{
    private static readonly string DataDir = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "cmux2");
    ...
    /// Uses fuzzy matching when query is provided, sorted by score then favorites.
    /// </summary>
    public List<Snippet> Search(string query)
    {
        lock (_lock)
        {
            ...
            var q = query.Trim();
            var ranked = FuzzyMatcher.RankMatches(
                _snippets,
                q,
                s => s.Name + " " + s.Category + " " + (s.Description ?? "") + " " + string.Join(" ", s.Tags));

            // Fall back to substring match for content (not included in fuzzy text above)
            var fuzzyIds = ranked.Select(r => r.Item.Id).ToHashSet();
            var contentMatches = _snippets
                .Where(s => !fuzzyIds.Contains(s.Id) &&
                    s.Content.Contains(q, StringComparison.OrdinalIgnoreCase))
                .ToList();

            return ranked.Select(r => r.Item)
