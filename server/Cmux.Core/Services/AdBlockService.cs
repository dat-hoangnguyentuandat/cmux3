...
public class AdBlockService
{
    ...
    public string GetCosmeticBootstrapScript()
    {
        return """
(function() {
  function cmuxInjectCosmetic(selectors) {
    if (!selectors || selectors.length === 0) return;
    var style = document.createElement('style');
    style.id = '__cmux_adblock__';
    style.textContent = selectors.map(function(s){ return s + '{display:none!important;visibility:hidden!important;}'; }).join('\n');
    (document.head || document.documentElement).appendChild(style);
  }

  function cmuxApplyCosmetic() {
    var host = location.hostname;
    window.chrome && chrome.webview && chrome.webview.postMessage(JSON.stringify({type:'cmux-cosmetic-req', host: host}));
  }

  window.chrome && chrome.webview && chrome.webview.addEventListener('message', function(e) {
    try {
      var msg = JSON.parse(e.data);
      if (msg.type === 'cmux-cosmetic-res') {
        cmuxInjectCosmetic(msg.selectors);
      }
    } catch(_) {}
  });

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', cmuxApplyCosmetic);
  } else {
    cmuxApplyCosmetic();
  }
})();
""";
    ...
    public string? BuildScriptletInjection(string hostname)
    {
        ...
        sb.Append("\nvar __cmuxSL=window.__cmuxScriptlets;\n");
        foreach (var r in rules)
        {
            ...
            sb.Append("try{__cmuxSL.run(").Append(nameJson).Append(',').Append(argsJson).Append(");}catch(e){}\n");
        }
        sb.Append("}catch(e){}})();");
        return sb.ToString();
    }
    ...
    /// window.__cmuxScriptlets with a run(name, args) dispatcher. Focused on the
    ...
    {
        return """
if (!window.__cmuxScriptlets) {
  var SL = window.__cmuxScriptlets = {};
  ...
  function hookFetch(transform){
    ...
    if (!orig || orig.__cmuxHooked) return;
  ...
  function hookXhr(transform){
    ...
    if (OrigOpen.__cmuxHooked) return;
    var open = function(method, url){ this.__cmuxUrl = url; return OrigOpen.apply(this, arguments); };
    open.__cmuxHooked = true;
    XMLHttpRequest.prototype.open = open;
    XMLHttpRequest.prototype.send = function(){
      var xhr = this;
      var url = xhr.__cmuxUrl || '';
