...
public sealed class DaemonClient : IDisposable
{
    ...
    private static string? FindDaemonExecutable()
    {
        ...
        // 2. Look in sibling publish output (local single-file publish scenario)
        try
        {
            var publishDir = new DirectoryInfo(appDir);
            while (publishDir != null && !string.Equals(publishDir.Name, "publish", StringComparison.OrdinalIgnoreCase))
                publishDir = publishDir.Parent;

            if (publishDir != null)
            {
                candidate = Path.Combine(publishDir.FullName, "cmux-daemon-win-x64-single", "cmux-daemon.exe");
                if (File.Exists(candidate)) return candidate;
            }
        }
        catch
        ...
        // 3. Look in sibling project build output (dev build scenario)
    ...
    private void DispatchEvent(DaemonEvent evt)
    {
        switch (evt.Type)
        {
            ...
            case DaemonMessageTypes.EventCwdChanged:
                if (evt.PaneId != null && evt.Data != null)
                {
                    LogDaemon($"[Listen] CWD_CHANGED pane={evt.PaneId} dir={evt.Data}");
