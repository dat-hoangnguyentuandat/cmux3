...
public class SplitNode
{
    ...
    /// <summary>
    /// Detaches the source pane from the tree and re-inserts it beside the
    /// target pane along the given direction. The target leaf becomes a
    /// container holding the target pane and the moved source pane.
    /// PaneIds are preserved so live sessions stay attached.
    /// When <paramref name="sourceFirst"/> is true the moved pane occupies the
    /// First slot (left/top); otherwise the Second slot (right/bottom).
    /// </summary>
    public bool MovePaneToEdge(string sourcePaneId, string targetPaneId, SplitDirection direction, bool sourceFirst)
    {
        if (sourcePaneId == targetPaneId) return false;

        var src = FindNode(sourcePaneId);
        var tgt = FindNode(targetPaneId);
        if (src is not { IsLeaf: true } || tgt is not { IsLeaf: true }) return false;

        // Detach the source leaf. Remove() collapses the source's parent into
        // its surviving sibling, so the target node object may be relocated —
        // re-resolve it afterwards rather than trusting the earlier reference.
        if (!Remove(sourcePaneId)) return false;

        var target = FindNode(targetPaneId);
        if (target is not { IsLeaf: true }) return false;

        var existing = CreateLeaf(targetPaneId);
        var moved = CreateLeaf(sourcePaneId);

        target.IsLeaf = false;
        target.Direction = direction;
        target.SplitRatio = 0.5;
        target.PaneId = null;
        target.First = sourceFirst ? moved : existing;
        target.Second = sourceFirst ? existing : moved;
        return true;
    }
}
