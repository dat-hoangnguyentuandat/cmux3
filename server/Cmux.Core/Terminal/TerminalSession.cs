...
public sealed class TerminalSession : IDisposable
{
    ...
    private bool _trackingEscapeSequence;
    private int _trackedEscapeLength;
        ...
        Dictionary<string, string>? extraEnv = null)
    {
        ...
        lock (_lock)
        {
            ...
            _process = new TerminalProcess(_console, command, effectiveWorkingDirectory, extraEnv);
    ...
    /// <summary>
    /// Feeds VT data directly into the parser without a ConPTY process.
    /// Used by tests to exercise the full session pipeline (parser → buffer → Redraw event)
    /// without spawning a real shell process.
    /// </summary>
    public void FeedForTesting(string data)
    {
        lock (_lock)
        {
            _parser.Feed(data);
    ...
    private void TrackInput(byte[] data)
    {
        var text = Encoding.UTF8.GetString(data);
        foreach (var ch in text)
        {
            if (_trackingEscapeSequence)
            {
                _trackedEscapeLength++;
                if (_trackedEscapeLength == 1 && ch is '[' or 'O')
                    continue;
                if (ch is >= '@' and <= '~')
                    _trackingEscapeSequence = false;
                continue;
            }
            ...
            {
                case '':
                    _trackingEscapeSequence = true;
                    _trackedEscapeLength = 0;
                    break;
                case '\r':
                case '\n':
                    var command = _inputBuffer.ToString().Trim();
                    var visibleCommand = TryGetVisibleCommandLine();
                    _inputBuffer.Clear();
                    if (!string.IsNullOrWhiteSpace(visibleCommand) && visibleCommand.Length > command.Length)
                ...
                default:
                    if (!char.IsControl(ch))
                    {
                        ...
                        if (_inputBuffer.Length > 4096)
                            _inputBuffer.Remove(0, _inputBuffer.Length - 4096);
                    }
                    break;
            }
        }
    ...
    private string? TryGetVisibleCommandLine()
    {
        lock (_lock)
        {
            var row = Buffer.CursorRow;
            var cols = Math.Min(Buffer.CursorCol, Buffer.Cols);
            if (row < 0 || row >= Buffer.Rows || cols <= 0)
                return null;

            var cells = Buffer.GetLine(row);
            var chars = new char[cols];
            for (var i = 0; i < cols; i++)
            {
                var ch = cells[i].Character;
                chars[i] = ch == '\0' ? ' ' : ch;
            }

            var line = new string(chars).TrimEnd();
            return ExtractCommandFromPromptLine(line);
        }
    }
    ...
    private static string? ExtractCommandFromPromptLine(string line)
    {
        ...
        var verbs = new[] { "cd", "chdir", "sl", "set-location", "pushd", "popd", "pwd" };
        foreach (var verb in verbs)
        {
            var index = trimmed.IndexOf(verb, StringComparison.OrdinalIgnoreCase);
            while (index >= 0)
            {
                var beforeOk = index == 0 || !IsCommandWordCharacter(trimmed[index - 1]);
                var after = index + verb.Length;
                var afterOk = after >= trimmed.Length || char.IsWhiteSpace(trimmed[after]) || trimmed[after] == '.' || trimmed[after] == '\\' || trimmed[after] == '/';
                if (beforeOk && afterOk)
                    return trimmed[index..].Trim();

                index = trimmed.IndexOf(verb, index + 1, StringComparison.OrdinalIgnoreCase);
            }
        }

        var promptIndex = trimmed.LastIndexOfAny(['>', '❯', '➜', '$', '#', 'λ', '❱', '❯', '➤', '▶', '→', '»']);
        if (promptIndex >= 0 && promptIndex + 1 < trimmed.Length)
            return trimmed[(promptIndex + 1)..].Trim();

        return trimmed;
    }

    private static bool IsCommandWordCharacter(char ch) =>
    ...
    /// <summary>
    /// Clears the visible screen + scrollback under the session lock,
    /// so it can't race the background read thread mutating the buffer.
    /// </summary>
