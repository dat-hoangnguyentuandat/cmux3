...
/// <summary>
/// Owns the lifecycle of the cmux3 web host (Cmux.Web). The launcher always
...
internal static class ServerManager
{
    ...
    internal static bool EnsureRunning()
    {
        ...
        if (psi == null)
        {
            Console.Error.WriteLine("Could not locate the cmux3 web host (cmux-web or Cmux.Web project).");
            return false;
        }

        Console.WriteLine("Starting cmux3 server...");
    ...
    private static ProcessStartInfo? BuildHostStartInfo()
    {
        ...
        foreach (var name in new[] { "cmux-web.exe", "Cmux.Web.exe" })
        {
    ...
    private static string? FindWebProject(string startDir)
    {
        ...
        while (dir != null)
        {
            var candidate = Path.Combine(dir.FullName, "server", "Cmux.Web", "Cmux.Web.csproj");
            if (File.Exists(candidate))
                return candidate;
            // Also handle running from inside the server tree.
            var sibling = Path.Combine(dir.FullName, "Cmux.Web", "Cmux.Web.csproj");
    ...
    internal static bool Stop()
    {
        if (!IsRunning())
        {
            Console.WriteLine("cmux3 server is not running.");
            return true;
        }
        ...
        try
        {
            if (File.Exists(PidFile) && int.TryParse(File.ReadAllText(PidFile).Trim(), out var pid))
            {
                try
                {
                    ...
                    File.Delete(PidFile);
                    Console.WriteLine("cmux3 server stopped.");
                    return true;
                }
