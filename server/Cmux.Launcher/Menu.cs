...
internal sealed class Menu
{
    ...
    internal Menu()
    {
        ...
        {
            ("Web UI (Open in Browser)", "Start the host and open cmux3 in your browser", OpenWeb),
            ("CLI (Interactive)",        "Open the interactive cmux command console",      OpenCli),
            ("Run in Background (Tray)", "Hide to the system tray, keep the server up",     OpenTray),
            ("Server: Start / Stop",     "Toggle the cmux3 web host",                       ToggleServer),
            ("Check for Updates",        "Check GitHub for a newer cmux3 release",          CheckUpdate),
    ...
    private void Render()
    {
        ...
        WriteCentered("c m u x 3", ConsoleColor.Cyan);
        Console.WriteLine();
    ...
    private bool Exit()
    {
        Console.WriteLine("Bye. The server keeps running; use 'cmux3 stop' to shut it down.");
        return false;
    }
