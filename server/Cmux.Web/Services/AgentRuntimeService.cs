...
public sealed class AgentRuntimeService : IDisposable
{
    ...
    private readonly AgentConversationStoreService _conversationStore;

    /// <summary>
    /// Optional handler bridging agent "cmux" tool calls to the local command
    /// surface. Mirrors the desktop App.PipeServer.OnCommand hook.
    /// </summary>
    public Func<string, Dictionary<string, string>, Task<string>>? CommandHandler { get; set; }

    public AgentRuntimeService(AgentConversationStoreService conversationStore)
    {
    ...
    private bool TryQueuePrompt(string prompt, AgentPaneContext context, string? explicitThreadId, bool echoToPane)
    {
        ...
        var savedUserMessage = _conversationStore.AppendMessage(new AgentConversationMessage
        {
            ThreadId = threadId,
            Role = "user",
        ...
        _ = Task.Run(async () =>
        {
            ...
            try
            {
                ...
                var assistantMessage = _conversationStore.AppendMessage(new AgentConversationMessage
                {
                    ThreadId = threadId,
                    Role = "assistant",
                    Content = finalText,
        ...
        CancellationToken ct)
    {
        ...
        {
            "anthropic" => await RunAnthropicConversationAsync(settings, ResolveAnthropicConfig(settings), userPrompt, systemPrompt, context, runKey, threadId, tools, historyContext.HistoryMessages, onAssistantDelta, ct),
            "custom" => await RunCustomProviderAsync(settings, userPrompt, systemPrompt, context, runKey, threadId, tools, historyContext.HistoryMessages, onAssistantDelta, ct),
            "gemini" => await RunOpenAiConversationAsync(settings, ResolveGeminiConfig(settings), userPrompt, systemPrompt, context, runKey, threadId, tools, historyContext.HistoryMessages, onAssistantDelta, ct),
            _ => await RunOpenAiConversationAsync(settings, ResolveOpenAiConfig(settings), userPrompt, systemPrompt, context, runKey, threadId, tools, historyContext.HistoryMessages, onAssistantDelta, ct),
        };
        ...
        CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(apiKey))
            return AgentRunResult.Error(config.MissingKeyMessage);

        var model = config.Model;
        var providerTag = config.ProviderTag;
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            if (useServerStream)
            {
                ...
                if (!streamed.Success)
                    return AgentRunResult.Error(streamed.Error ?? "Streaming request failed.", providerTag, model);
            ...
            else
            {
                ...
                if (!res.IsSuccessStatusCode)
                    return AgentRunResult.Error($"Model request failed ({(int)res.StatusCode}): {Truncate(body, 800)}", providerTag, model);
            ...
            if (toolCalls.Count == 0)
            {
                ...
                if (!string.IsNullOrWhiteSpace(assistantText))
                {
                    return new AgentRunResult(assistantText, providerTag, model, inputTokens, outputTokens, totalTokens);
                }

                return new AgentRunResult("No text response received.", providerTag, model, inputTokens, outputTokens, totalTokens);
            }
        ...
        return new AgentRunResult("Stopped after too many tool iterations.", providerTag, model, inputTokens, outputTokens, totalTokens);
    }
        ...
        CancellationToken ct)
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            if (toolCalls.Count == 0)
            {
                ...
                return new AgentRunResult(finalText, providerTag, model, inputTokens, outputTokens, totalTokens);
            }
...
""");
    ...
    private async Task<string> ExecutePipeCommandAsync(string command, Dictionary<string, string> args, CancellationToken ct)
    {
        ct.ThrowIfCancellationRequested();
...
- paneType: {context.PaneTypeLabel ?? "terminal"}
{(string.IsNullOrWhiteSpace(context.PaneContent) ? "" : $"- paneContent:\n{Truncate(context.PaneContent, 4000)}\n")}Use tools whenever they can produce reliable results.
...
""";
    ...
    private string ResolveThreadId(AgentPaneContext context, AgentSettings settings, string runKey, string? explicitThreadId)
    {
        ...
        {
            var existing = _conversationStore.GetThread(explicitThreadId.Trim());
            if (existing != null)
                return existing.Id;
        }
        ...
        {
            var existing = _conversationStore.GetThread(activeThreadId);
            if (existing != null)
                return existing.Id;
        }

        var created = _conversationStore.CreateThread(
            context.WorkspaceId,
            context.SurfaceId,
            context.PaneId,
            settings.AgentName);
    ...
    private static string ResolveModel(AgentSettings settings)
    {
        ...
        return provider switch
        {
            ...
            "gemini" => string.IsNullOrWhiteSpace(settings.Gemini.Model) ? "gemini-2.0-flash" : settings.Gemini.Model.Trim(),
            "custom" => ResolveCustomProvider(settings)?.Model?.Trim() is { Length: > 0 } cm ? cm : "claude-sonnet-4-6",
            _ => string.IsNullOrWhiteSpace(settings.OpenAi.Model) ? "gpt-4o-mini" : settings.OpenAi.Model.Trim(),
        };
    }

    private static CustomProviderSettings? ResolveCustomProvider(AgentSettings settings)
    {
        var providers = settings.CustomProviders ?? [];
        if (providers.Count == 0)
            return null;

        var name = (settings.ActiveCustomProviderName ?? "").Trim();
        if (!string.IsNullOrWhiteSpace(name))
        {
            var match = providers.FirstOrDefault(p => string.Equals(p.Name, name, StringComparison.OrdinalIgnoreCase));
            if (match != null)
                return match;
        }
    ...
    private static ProviderHttpConfig ResolveOpenAiConfig(AgentSettings settings)
    {
        return new ProviderHttpConfig(
            ProviderTag: "openai",
            BaseUrl: EnsureAbsoluteBase(settings.OpenAi.BaseUrl, "https://api.openai.com/v1"),
            Model: string.IsNullOrWhiteSpace(settings.OpenAi.Model) ? "gpt-4o-mini" : settings.OpenAi.Model.Trim(),
            ApiKey: SecretStoreService.GetSecret(settings.OpenAi.ApiKeySecretName),
            MissingKeyMessage: "OpenAI-compatible API key is not set in Settings -> Agent.",
            ApplyAuthHeaders: req => req.Headers.Authorization = new AuthenticationHeaderValue("Bearer", SecretStoreService.GetSecret(settings.OpenAi.ApiKeySecretName)));
    }

    private static ProviderHttpConfig ResolveGeminiConfig(AgentSettings settings)
    {
        return new ProviderHttpConfig(
            ProviderTag: "gemini",
            BaseUrl: EnsureAbsoluteBase(settings.Gemini.BaseUrl, "https://generativelanguage.googleapis.com/v1beta/openai"),
            Model: string.IsNullOrWhiteSpace(settings.Gemini.Model) ? "gemini-2.0-flash" : settings.Gemini.Model.Trim(),
            ApiKey: SecretStoreService.GetSecret(settings.Gemini.ApiKeySecretName),
            MissingKeyMessage: "Gemini API key is not set in Settings -> Agent.",
            ApplyAuthHeaders: req => req.Headers.Authorization = new AuthenticationHeaderValue("Bearer", SecretStoreService.GetSecret(settings.Gemini.ApiKeySecretName)));
    }

    private static ProviderHttpConfig ResolveAnthropicConfig(AgentSettings settings)
    {
        return new ProviderHttpConfig(
            ProviderTag: "anthropic",
            BaseUrl: EnsureAbsoluteBase(settings.Anthropic.BaseUrl, "https://api.anthropic.com"),
            Model: string.IsNullOrWhiteSpace(settings.Anthropic.Model) ? "claude-3-5-sonnet-latest" : settings.Anthropic.Model.Trim(),
            ApiKey: SecretStoreService.GetSecret(settings.Anthropic.ApiKeySecretName),
            MissingKeyMessage: "Anthropic API key is not set in Settings -> Agent.",
            ApplyAuthHeaders: req =>
            {
                var key = SecretStoreService.GetSecret(settings.Anthropic.ApiKeySecretName);
                req.Headers.Add("x-api-key", key);
                req.Headers.Add("anthropic-version", "2023-06-01");
            });
    }

    private static ProviderHttpConfig ResolveCustomProviderConfig(CustomProviderSettings provider)
    {
        var kind = (provider.Kind ?? "anthropic").Trim().ToLowerInvariant();
        var defaultBase = kind == "openai" ? "https://api.openai.com/v1" : "https://api.anthropic.com";
        var defaultModel = kind == "openai" ? "gpt-4o-mini" : "claude-sonnet-4-6";
        var authScheme = (provider.AuthScheme ?? "bearer").Trim().ToLowerInvariant();
        if (authScheme is not ("bearer" or "x-api-key"))
            authScheme = "bearer";

        var anthropicVersion = string.IsNullOrWhiteSpace(provider.AnthropicVersion) ? "2023-06-01" : provider.AnthropicVersion.Trim();
        var providerName = string.IsNullOrWhiteSpace(provider.Name) ? "custom" : provider.Name.Trim();

        return new ProviderHttpConfig(
            ProviderTag: $"custom:{providerName}",
            BaseUrl: EnsureAbsoluteBase(provider.BaseUrl, defaultBase),
            Model: string.IsNullOrWhiteSpace(provider.Model) ? defaultModel : provider.Model.Trim(),
            ApiKey: SecretStoreService.GetSecret(provider.ApiKeySecretName),
            MissingKeyMessage: $"API key for custom provider '{providerName}' is not set in Settings -> Agent.",
            ApplyAuthHeaders: req =>
            {
                var key = SecretStoreService.GetSecret(provider.ApiKeySecretName);
    ...
    private async Task<AgentRunResult> RunCustomProviderAsync(
        AgentSettings settings,
        string userPrompt,
        string systemPrompt,
        AgentPaneContext context,
        ...
        CancellationToken ct)
    {
        var provider = ResolveCustomProvider(settings);
        if (provider == null)
            return AgentRunResult.Error("No custom providers configured. Add one in Settings -> Agent.");

        var config = ResolveCustomProviderConfig(provider);
        var kind = (provider.Kind ?? "anthropic").Trim().ToLowerInvariant();

        return kind == "openai"
            ? await RunOpenAiConversationAsync(settings, config, userPrompt, systemPrompt, context, runKey, threadId, tools, historyMessages, onAssistantDelta, ct)
            : await RunAnthropicConversationAsync(settings, config, userPrompt, systemPrompt, context, runKey, threadId, tools, historyMessages, onAssistantDelta, ct);
    }
    ...
    private AgentConversationContext PrepareConversationContext(AgentSettings settings, string threadId, string currentPrompt, string systemPrompt)
    {
        ...
        var history = _conversationStore.GetMessages(threadId, maxMessages * 3)
        ...
        if (settings.AutoCompactContext && needsCompaction && history.Count > keepRecentOnCompaction + 2)
        {
            ...
            {
                var summaryMessage = _conversationStore.AppendMessage(new AgentConversationMessage
                {
                    ThreadId = threadId,
                    Role = "system",
                    Content = summary,
    ...
    private sealed record ProviderHttpConfig(
        string ProviderTag,
        string BaseUrl,
        string Model,
        string? ApiKey,
        string MissingKeyMessage,
        Action<HttpRequestMessage> ApplyAuthHeaders);
...
public sealed class AgentPaneContext
{
    ...
    /// <summary>For WebView panes: the current URL. For Notepad panes: the full text content.</summary>
    public string? PaneContent { get; init; }
    /// <summary>Human-readable pane type label for the system prompt.</summary>
    public string? PaneTypeLabel { get; init; }
}
